...
| `monoenergetic_x` | Select one normalized kinetic energy, `x=E/T`. | Default `-1` performs the Maxwellian velocity integral; a positive value returns the delta-normalized monoenergetic kernel at `u=v/vth=sqrt(x)`. |
