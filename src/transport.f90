...
! original contains follows
    ...
    pure function Tphi_int(ux, taub, Hmn2)
        ...
        Tphi_int = sign(1.0_dp, psi_pr*q*sign_theta)*pi**(3.0_dp/2.0_dp)*mph**2*ni1*c*vth/qi &
    ...
    subroutine timestep_transport(v, eta, neq, t, y, ydot)
        ...
        if (eta > etatp) then
            ...
            t0 = 0.0_dp
            Hn = (2.0_dp - eta*bmod)*epsn*exp(imun*(q*mph*(y(1)) - mth*(t - t0)*Omth))
        else
            Hn = (2.0_dp - eta*bmod)*epsn*exp(imun*(q*mph*(y(1)) - (mth + q*mph)*t*Omth))
        end if
        ydot(3) = real(Hn)
        ydot(4) = aimag(Hn)
