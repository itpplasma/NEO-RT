...
contains
    ...
    subroutine manufactured_get_components(provider, reference, h0, jperp, &
        ...
        if (provider%zero_width_mode) then
            ...
            components(1) = gc_nonlocal_component_t(component_id=101, &
                sigma=1, x_min=-provider%width, x_max=provider%width)
            components(2) = gc_nonlocal_component_t(component_id=202, &
                sigma=-1, x_min=-provider%width, x_max=provider%width)
        else
            components(1) = gc_nonlocal_component_t(component_id=101, &
