...
!
  module find_all_roots_mod
    ...
    double precision :: relerr=1.d-11 !1.d-12
    double precision, dimension(:), allocatable :: xcustom,roots
  end module find_all_roots_mod
!
  module global_invariants
    double precision :: rout,dtau,toten,perpinv,sigma
    double precision :: Phi_eff
  end module global_invariants
!
  module poicut_mod
    integer :: npc
    double precision :: rpc_beg,h_rpc,rmagaxis,zmagaxis,psimagaxis
    double precision :: rhopol_bou,psi_bou,Rbou_lfs,Zbou_lfs,Rbou_hfs,Zbou_hfs
...
!
! computes alpha_lifetime phase space coordinates as functions of
...
!
  ...
  rho_pol=0.9d0 !0.99999d0             CAUTION: range of Poincare cut limited because of poor data near separatrix
...
!
  use find_all_roots_mod, only : nsearch_min,niter,relerr,    &
...
!
  if(iter.gt.niter) then
    print *,'No convergence of Newton in find_all_roots'
    ierr=1
  endif
!
  roots(nroots)=x
...
!
  use sample_matrix_mod, only : nlagr,n1,n2,npoi,xarr,amat_arr
  use get_matrix_mod,    only : iclass
